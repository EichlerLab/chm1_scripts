...
cc <- c("character", "integer", "integer", "character")
cn <- c("chrom", "pos", "end", "mut")
...
countsByChr <- lapply(seq(1,nChr), function(i) hist(strByChr[[i]]$pos, breaks=c(seq(0,faiTab$V2[i], by=args$size),faiTab$V2[i]), plot=F))
