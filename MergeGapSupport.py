...
#    vals[3] = 1
    ...
    try:   
        if (prevCoords is not None and chrom == prevCoords[0] and overlap > args.overlap):
            vals = [chrom, min(start, prevCoords[1]), max(end, prevCoords[2]), (float(vals[3])*int(vals[4]) + float(prevVals[3])*int(prevVals[4])) / (int(vals[4]) + int(prevVals[4])), int(vals[4])+ int(prevVals[4]), vals[5], ";".join([prevVals[6], vals[6]]), ";".join([prevVals[7], vals[7]]), ";".join([prevVals[8], vals[8]])]
